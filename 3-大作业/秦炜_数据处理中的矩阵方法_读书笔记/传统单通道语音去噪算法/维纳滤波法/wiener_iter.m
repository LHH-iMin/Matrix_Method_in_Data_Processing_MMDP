function [enhanced_speech] = wiener_iter(noisyfile, enhancedfile, iter_num)
...
[noisy_speech, Fs]= audioread( noisyfile);	%nsdata is a column vector
...
noisedata= noisy_speech( 1: nwind* NF_SABSENT);	noise_colindex= 1+ ( 0: NF_SABSENT- 1)* nwind;
...
x= noisy_speech( nwind* NF_SABSENT+ 1: end);	%x is to-be-enhanced noisy speech
...
for k= 1: ncol
   ...
   enhanced_speech(colindex( k): colindex( k)+ nwind- 1)= [es_tmp( 1: noverlap)+ es_old;es_tmp( noverlap+ 1: nwind)];
   %overlap-add
   es_old= es_tmp( nwind- noverlap+ 1: nwind);
end

% wavwrite( enhanced_speech, Fs, bits, enhancedfile);
