...
[enhanced_speech] = wiener_wt('sp_noisy.wav','sp_enhanced.wav');
...
subplot(325);
plot(t,real(enhanced_speech)); ylim([-1.5,1.5]); title('(c)维纳小波法-增强语音'); xlabel('时间/s'); ylabel('幅度');
%-------------------------------语谱图----------------------------------------
subplot(322);
...
subplot(326);
spectrogram(enhanced_speech,256,128,256,8000,'yaxis');xlabel('时间(s)');ylabel('频率(Hz)');title('(f)维纳小波法-增强语音语谱图');
