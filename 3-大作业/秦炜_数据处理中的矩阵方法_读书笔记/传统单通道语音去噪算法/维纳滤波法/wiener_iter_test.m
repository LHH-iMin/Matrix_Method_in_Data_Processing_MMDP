...
[enhanced_speech] = wiener_iter('sp_noisy.wav','sp_enhanced.wav',4);

%-----------------------信噪比--------------------------------------------\

SNR_before = snr(signal,noise)
res = signal(1:length(enhanced_speech)) - enhanced_speech';
SNR_after = snr(signal(1:length(enhanced_speech)),res)
%-------------------------------波形----------------------------------------
figure;
t = (0:length(enhanced_speech)-1)/fs;
subplot(321);
plot(t,signal(1:length(enhanced_speech)),'k'); ylim([-1.5,1.5]); 
title('(a)纯净语音'); xlabel('时间/s'); ylabel('幅度');
subplot(323);
plot(t,sp_noisy(1:length(enhanced_speech)),'b');ylim([-1.5,1.5]);
title('b)带噪语音(5dB白噪声)'); xlabel('时间/s');ylabel('幅度');
subplot(325);
plot(t,real(enhanced_speech),'r'); ylim([-1.5,1.5]); 
title('(c)维纳iter法-增强语音'); xlabel('时间/s'); ylabel('幅度');
%-------------------------------语谱图----------------------------------------
subplot(322);
...
subplot(326);
spectrogram(enhanced_speech,256,128,256,8000,'yaxis');xlabel('时间(s)');ylabel('频率(Hz)');title('(f)维纳iter法-增强语音语谱图');
