function [enhanced_speech] = logmmse(noisyfile,enhancedfile)
...
%  
%
%  Example call:  [enhanced_speech] = logmmse('sp_noisy.wav','sp_enhanced.wav');
%
%  References:
...
[noisy_speech, Srate] = audioread(noisyfile);	%nsdata is a column vector
...
j=1;
for m=1:6
    noise_mean=noise_mean+abs(fft(win.*noisy_speech(j:j+len-1),nFFT));
    j=j+len;
end
...
x_old=zeros(len1,1);
Nframes=floor(length(noisy_speech)/len2)-floor(len/len2);
% enhanced_speech=zeros(Nframes*len2,1);
...
for n = 1:Nframes
    ...
    insign = win.*noisy_speech(k:k+len-1);
    ...
    enhanced_speech(k:k+ len2-1) = x_old+ xi_w(1:len1);
...
% audiowrite(enhanced_speech,Srate,16,enhancedfile);
audiowrite(enhancedfile, enhanced_speech, Srate);
